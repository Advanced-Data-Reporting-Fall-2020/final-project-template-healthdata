...
```{r}
head(CompiledLHDExpenditures)
```
```{r}
PopulationSorted <- arrange(CompiledLHDExpenditures, desc(population))
```


Finding Average per capita spending
```{r}
CompiledLHDExpenditures %>% summarize(mean(expenditures, na.rm=TRUE))
```

```{r}
median(CompiledLHDExpenditures, na.rm = FALSE)
```
Trying to convert per capita column from char type to numeric
```{r}
CompiledLHDEXpenditures<- as.numeric(CompiledLHDExpenditures$per_capita_spending)
```
```{r}
...
```{r}
as.numeric(CompiledLHDExpenditures$per_capita_spending)
```


```{r}
as.numeric(CompiledLHDExpenditures$expenditures)
```
```{r}
head(CompiledLHDExpenditures)
```
...
```{r}
CompiledLHDExpenditures %>% mean(expenditures)
```
