...
single_counties_summary_nc %>%
  group_by(lhd_name) %>%
  summarise(Avg_fte = mean(fte)) %>%
  select(lhd_name, Avg_fte) %>%
...
Highest and lowest fte per capita in 2017
```{r}
fte_per_capita %>%
  filter(year == "2017") %>%
...
```{r}
ggplot(fte_per_capita, aes(x=population, y=fte)) + 
  geom_line() +
  facet_wrap(~year, ncol=4) +
  theme(axis.text.x = element_text(angle = 45, hjust=1)) +
  labs(title = "FTE Change Over Time", y ="Number of Employees", x = "Population")
```
